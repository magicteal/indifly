...
export default function HeroSection({ hero, service }: HeroSectionProps) {
  ...
  return (
    ...
    >
      <Container className="mt-20 mb-24 flex flex-col-reverse items-center gap-12 md:mt-40 md:mb-40 md:flex-row md:justify-between md:gap-16 lg:gap-24">
        {/* Left copy block */}
        <div className="reveal-left relative z-10 w-full max-w-xl text-center md:max-w-[560px] md:text-left">
          <div className="text-xl font-semibold tracking-wide sm:text-2xl">
            <span>in</span>
            <span className="text-primary">{hero.text[0]}</span>
          </div>

          <h1 className="reveal-title mt-4 text-4xl leading-tight font-medium sm:text-5xl md:text-6xl/[1.1]">
            <span className="block">{hero.text[1]}</span>
            <span className="mt-2 block font-bold italic">{hero.text[2]}</span>
          </h1>
        ...
        {/* Right artwork */}
        {HeroArt && (
          <div className="reveal-image relative w-full max-w-[480px] sm:max-w-[560px] md:max-w-[520px] lg:max-w-[600px] xl:max-w-[640px]">
            <HeroArt
